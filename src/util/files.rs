...
pub fn verify_files() {
  let game_settings_file = fs::read_to_string(GAME_SETTING_PATH).unwrap_or_else(|_| panic!("Failed to open file {}", GAME_SETTING_PATH));
  let level_file = fs::read_to_string(LEVEL_FILE_PATH).unwrap_or_else(|_| panic!("Failed to open file {}", LEVEL_FILE_PATH));
  let sprite_file = fs::read_to_string(SPRITE_FILE_PATH).unwrap_or_else(|_| panic!("Failed to open file {}", SPRITE_FILE_PATH));
  let sprite_types_file = fs::read_to_string(SPRITE_TYPE_FILE_PATH).unwrap_or_else(|_| panic!("Failed to open file {}", SPRITE_TYPE_FILE_PATH));
  ...
  let found_problems = toml_problems.iter().filter(|file| file.is_some()).map(|file| file.clone().unwrap());
