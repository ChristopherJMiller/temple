...
fn bootstrap_game(mut commands: Commands, query: Query<Entity, With<BeginGame>>, game_file: Res<GameFile>, mut temple_state: ResMut<TempleState>) {
    ...
    match game_file.level_transistion {
      ...
      LevelTransistionType::NoOverworld => {
        if let Some(level_order) = &game_file.level_order {
          let level = *level_order.first().unwrap();
